...
const int wait_rest_time = 20 * 50;  //3s*50Hz, the time wait for automatic rest 
const int bluetooth_wait_time = 1 * 5000;   
...
static const unsigned long REFRESH_INTERVAL_EXECUTION = 300;
/* ---------------------------------------------------------------------------*/
...
//  Serial.println("Waiting for radio signal");
          ...
          if (order <= 8 && order >= 1)
          {
            if (!is_stand() && order != 5 && order != 8)
              stand();

          switch (order)
          {
          case 1:
            writeBluetooth("Step forward");
            step_back(1);
            break;
          case 2:
            writeBluetooth("Step back");
            step_forward(1);
            break;
          case 3:
            writeBluetooth("Turn left");
            turn_left(1);
            break;
          case 4:
            writeBluetooth("Turn right");
            turn_right(1);
            break;
          case 5:
            if (is_stand())
            ...
            {
              writeBluetooth("Stand");
              stand();
            }
            break;
          case 6:
            writeBluetooth("Dance a little");
            move_speed = 1.4;
            dance();
            break;
          case 7:
    ...
    {
      if (is_stand()  && !freeRoam)
      {
        writeBluetooth("Auto sit");
        sit();
        rest_counter = 0;
      }
    } 
...
boolean readyToExecute()
{
  ...
  if(millis() - lastRefreshTime >= REFRESH_INTERVAL_EXECUTION)
  {
    lastRefreshTime += REFRESH_INTERVAL_EXECUTION;
    return true;
  } else {
    return false;
  }
